...
shape_colors = [GREEN, RED, BLUE, WHITE, (255, 165, 0), (0, 0, 255), (128, 0, 128)]
...
def draw_grid(surface, grid):
    ...
    for i in range(len(grid)):
        pygame.draw.line(surface, WHITE, (sx, sy + i * BLOCK_SIZE), (sx + PLAY_WIDTH, sy + i * BLOCK_SIZE))  # horizontal lines
        for j in range(len(grid[i])):
            pygame.draw.line(surface, WHITE, (sx + j * BLOCK_SIZE, sy), (sx + j * BLOCK_SIZE, sy + PLAY_HEIGHT))  # vertical lines
...
def main():
    ...
    while run:
        ...
        if change_piece:
            ...
            current_score += calculate_score(lines_cleared) 
            

        draw_window(win, grid, current_score) 
        draw_next_shape(next_piece, win)
        pygame.display.update()

        if check_lost(locked_positions):
            draw_text_middle(win, f"YOU LOST! Счет: {current_score}", 50, WHITE)
            pygame.display.update()
